...
namespace Moments
{
	...
	public sealed class Recorder : MonoBehaviour
	{
		...
		Queue<Action> m_MainThreadActions;
		object m_MainThreadActionsLock = new object();
...
void EnsureInternalState()
        {
            ...
            if (m_ReflectionUtils == null)
                m_ReflectionUtils = new ReflectionUtils<Recorder>(this);

            if (m_MaxFrameCount == 0)
                m_MaxFrameCount = Mathf.RoundToInt(m_BufferSize * m_FramePerSecond);

            if (m_TimePerFrame == 0f)
                m_TimePerFrame = 1f / m_FramePerSecond;
        }

        internal void CaptureFrame(CommandBuffer cmd, RenderTargetIdentifier source)
        {
            EnsureInternalState();
		...
		void Init()
		{
			...
			if (string.IsNullOrEmpty(SaveFolder))
			{
				...
				#elif UNITY_ANDROID
				SaveFolder = GetAndroidGalleryPath();
				#else
		...
		static string GetAndroidGalleryPath()
		{
			try
			{
				using (AndroidJavaClass envClass = new AndroidJavaClass("android.os.Environment"))
				{
					string picturesDir = envClass.GetStatic<string>("DIRECTORY_PICTURES");
					using (AndroidJavaObject externalDir = envClass.CallStatic<AndroidJavaObject>("getExternalStoragePublicDirectory", picturesDir))
					{
		...
		static void RefreshAndroidGallery(string filepath)
		{
			...
			try
			{
				...
				using (AndroidJavaObject activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity"))
				{
					...
					string[] mimeTypes = new string[] { "image/gif" };
		...
		IEnumerator PreProcess(string filename)
		{
			...
			Worker worker = new Worker(WorkerPriority)
			{
				...
				m_OnFileSaved = (id, path) => EnqueueMainThreadAction(() =>
				{
					if (OnFileSaved != null)
						OnFileSaved(id, path);
